...
| 插件 ID | 名称 | 类型 | 适合场景 |
| --- | --- | --- | --- |
| `agent.loop.react` | ReAct 智能体循环 | Agent Loop | 让 Agent 使用模型、工具、记忆和上下文能力完成多轮任务。 |
...
| `skill.registry` | 技能注册表 | 技能 | 加载本地 `SKILL.md`，让 Agent 搜索和注入相关技能。 |
...
| `workspace.sandbox` | 受限工作区 | 工具 | 在指定工作区内进行文件读写、搜索和受限命令执行。 |
...
- 作用：基于模型推理、工具发现、工具调用、记忆读取和记忆写入运行 Agent 对话。
- 提供能力：`agent.run`、`agent.stream`。
- 依赖能力：`model.chat`、`memory.query`、`memory.write`、`tool.registry.list`、`tool.invoke`。
- 资源：`agent_loop: react`。
- 配置要点：可设置 system prompt；可配置最大推理轮数、工具调用超时、触发上下文压缩的消息阈值。
...
- 作用：加载本地 `SKILL.md` 技能文件，并提供技能列表、读取和搜索能力。
- 提供能力：`skill.list`、`skill.get`、`skill.search`。
- 依赖能力：无。
- 资源：`skill: registry`。
- 配置要点：可配置 `skill_dirs`，指向一个或多个技能目录。
- 使用建议：让 Agent Loop 通过 `skill.search` 选择相关技能，再通过 `skill.get` 注入具体技能内容。
