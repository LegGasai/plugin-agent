...
class ContextManagerPlugin(PluginBase):
    def invoke(self, capability: str, payload: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        compressed = self.kernel.invoke(
            "context.compressor.compress",
            {"messages": messages},
            context,
        ).payload
        summary = str(compressed.get("summary", ""))
        replacement = self._replacement_messages(summary, messages, preserve_tail, summary_prefix)
        model_replacement = self._replacement_messages(summary, model_messages, preserve_tail, summary_prefix)
        return {
            "summary": summary,
            "messages": replacement,
            "model_messages": model_replacement,
